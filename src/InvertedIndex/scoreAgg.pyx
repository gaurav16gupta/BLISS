...
cdef extern from "Gather.h":
    cdef cppclass FastIV:
        FastIV(int, int, int, int, int ,int ,int, int*, int*) except +
        int Parts, R, B, mf, topk, node
        int block
        int* inv_lookup
        int* counts
...
cdef class PyFastIV:
    cdef FastIV *thisptr
    def __cinit__(self, int Parts, int R, int block, int B, int mf, int topk, int node, np.ndarray[int, ndim=1,  mode="c"] inv_lookup, np.ndarray[int, ndim=1,  mode="c"] counts ):
        self.thisptr = new FastIV(Parts, R, block, B, mf, topk, node, &inv_lookup[0], &counts[0])
